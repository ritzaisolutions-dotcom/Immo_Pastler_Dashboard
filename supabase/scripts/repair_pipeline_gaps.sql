-- Pipeline-Reparatur: fehlende Todos/Partner-Entwürfe + Zuordnung RPC + E-Mail-Status
-- Idempotent — Supabase SQL Editor oder: supabase db execute -f supabase/scripts/repair_pipeline_gaps.sql

-- ─── 1: Zuordnung RPC — Objekt per Adresse über alle Inserate ───────────────
...
BEGIN
  ...
  -- Adresse/PLZ: alle Objekte; Stadt allein nur bei passendem Vermieter
  FOR r IN
...
-- ─── 2: Zuordnung auf allen E-Mails neu berechnen ───────────────────────────
WITH resolved AS (
  SELECT
    id,
...
-- ─── 3: E-Mail #005 — verarbeitet mit bestehendem Todo synchronisieren ──────
...
-- ─── 4: Fehlende Todos für E-Mails #006–#008 ────────────────────────────────
...
-- ─── 5: Partner-Entwurf für Mülltonnen (#004) ───────────────────────────────
...
-- ─── 6: Verknüpfte Todos mit E-Mail-Zuordnung synchronisieren ───────────────
